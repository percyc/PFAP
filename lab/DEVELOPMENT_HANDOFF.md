...
## 2026-09-09 隐私账户持久化修复及真实重启验收

后续用户要求继续 100 节点实验。已更新 `run-live-hour.cjs`：显式验证运行包 SHA、恢复标记绑定版本、校验 100 个独立服务器/5 个跨组矿工/94 个交易节点、拒绝与其他活动实验共用服务器、准备节点按宿主机轮转。4 项策略测试通过。**尚未启动新的 100 节点实验，也未停止旧网络**：旧运行包部分密钥只在内存，停止旧 100 节点会使这些旧账户失去继续使用的能力；已向用户明确询问是否接受此代价，保留磁盘目录和报告不等于保全未落盘的密钥。等待其明确选择，不能把此前笼统“继续”当作知情同意删除内存中的唯一密钥。

- `e149d03`：原随机 AccountSK、下一 SN 状态与签名交易哈希原子持久化，文件/目录 fsync 完成后才广播；付款证明返回前保存冻结状态。节点内匿名 RPC 互斥，下一笔要求当前承诺在规范链上。拒绝旧密钥回退、重复初始化和无条件回滚；不自动重发未知交易。
- 独立 `exp-627c5ea52f82` 4 节点真实验证通过：9 笔交易全部 confirmed，两个交易节点各两次 SIGKILL 后均成功恢复并继续交易，双向 Transfer 和 Redeem 完成，最终余额准确。8 个 pk/vk 未变。详见 `VALIDATION_DURABLE_ACCOUNT_20260909.md`，证据 `lab/data/durable-account.sCZj85/`。
- 控制器 PID 1267169，新 runtime SHA `85e1e473abd3fe3d6c56e429e56d4e0e44ece170fb12761f609e4e8cac7b6f71`。旧 100 节点运行包与账户未修改。本轮是功能恢复验收，仍未完成 100 节点一小时测量；后续应使用健康新账户，保留原未知交易，不得将旧网络冒充新运行包验证。

## 2026-09-09 100 节点实验：基础设施阻塞，未完成一小时

**13:20 更新：** db3 网络及 20 个 geth 已恢复，100 节点在线、5 矿工运行。`4edf272` 修复恢复操作被日志维护抢占，race 测试通过，已与监控分队列一起发布（控制器 PID 1241424）。仍不能续跑：两笔查到成功回执进入双方状态核验，其中 node-65 本地余额仍旧；另七笔无哈希且付款承诺未就绪。源码确认原随机 AccountSK 仅保存在内存，SN 恢复不包含它。保留未知状态和原目录，不重发、不初始化；需要解决隐私状态持久化/崩溃恢复并明确新的验证路径。中断报告已导出，详见验收文档最新检查点。实验网络仍运行，无自动负载；不要将历史“离线”结论当成当前现场。
